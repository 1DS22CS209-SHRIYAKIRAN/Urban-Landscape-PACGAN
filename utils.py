...
def ROC_AUC(
    ...
    # Convert labels to long type and one-hot encode
    labels = labels.long()
    
    # Check if labels contain only one class
    if labels.sum() == 0 or labels.sum() == len(labels):
        # If all labels are the same, return precedent value or 0
        if len(precedent) == 0:
            return torch.tensor(0.0, device=device)
        else:
            return torch.tensor(precedent[-1], device=device)
    
    # One-hot encode labels
    labelsOH = F.one_hot(labels, num_classes=class_size).float().cpu().detach().numpy()
    
    # Apply Softmax to the predictions
    SoftMax = torch.nn.Softmax(dim=1)
    cls_SM = SoftMax(cls).cpu().detach().numpy()
    
    AUC = 0
    for i in range(class_size):
        # Handle potential issues with roc_auc_score
        try:
            AUC_i = roc_auc_score(labelsOH[:, i], cls_SM[:, i])
            AUC_i = torch.tensor(AUC_i, device=device)
        except ValueError:
            AUC_i = torch.tensor(0.0, device=device)
        AUC += AUC_i
