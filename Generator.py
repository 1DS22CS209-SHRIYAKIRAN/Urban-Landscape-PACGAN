...
class Generate():
    ...
    def Gen(self, 
        ...
        print('\nSaving generated images as PNG files...', flush=True)
        ...
        # Unnormalize and save images
        Images = (Images + 1) / 2.0  # Normalize from [-1, 1] to [0, 1]
        for i in range(n_images):
            save_image(Images[i], os.path.join(self.image_path, f"image_{i}.png"))

        print(f'\n=> Saved the synthetic images in {self.image_path}', flush=True)
        ...
        print(f'=> Saved the labels of the synthetic images in {self.path_labels}', flush=True)
